...
public class ObjectFactoryIdTests
{
    ...
    public class AutoIdGenerationTests
    {
        ...
        public void SetOne_WithoutId_ShouldGenerateAutoId()
        {
            ...
            Assert.StartsWith("obj_", registeredId);
        }
        ...
        public void AutoGeneratedIds_ShouldFollowPattern() // "obj_1", "obj_2", etc.
        {
            ...
            Assert.Matches(@"^obj_\d+$", id1);
            Assert.Matches(@"^obj_\d+$", id2);
            Assert.Matches(@"^obj_\d+$", id3);
        }
    }
    ...
    public class ThreadingAndCleanupTests
    {
        ...
        public void Registry_ShouldBeThreadSafe()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                tasks.Add(Task.Run(() =>
                {
                    factory.Register(objects[index], $"obj_{index}");
                }));
            }
            ...
            for (int i = 0; i < 100; i++)
            {
                var retrievedObj = factory.GetRegisteredObject<TestService>($"obj_{i}");
