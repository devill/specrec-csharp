...
{
    /// <summary>
    /// Discovers test cases for the [SpecRec] attribute by finding verified files and creating test cases
    /// that use SpecRecExecutor for unified execution with automatic cleanup and return value capture.
    /// </summary>
    public class SpecRecDiscoverer : IDataDiscoverer
    {
        ...
        private object[] CreateTestCaseData(IMethodInfo testMethod, CallLog callLog, string? verifiedFilePath = null)
        {
            ...
            // Create Context as first parameter (SpecRec tests always start with Context)
            ...
            for (int i = 1; i < methodParams.Length; i++)
            {
                ...
                {
                    // For methods with only Context parameter, skip preamble matching
        ...
        private string GenerateSuggestedPreamble(IParameterInfo[] methodParams)
        {
            ...
            // Skip the first parameter (Context) and generate placeholders for the rest
            for (int i = 1; i < methodParams.Length; i++)
            {
                var paramInfo = methodParams[i];
                var paramName = paramInfo.Name ?? "arg" + i;
