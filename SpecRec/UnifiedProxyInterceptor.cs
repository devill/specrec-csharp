...
namespace SpecRec
{
    ...
    /// <summary>
    /// Mixin class that implements IConstructorCalledWith for Castle.DynamicProxy
    /// </summary>
    public class ConstructorLoggingMixin : IConstructorCalledWith
    {
        private readonly CallLogger _logger;
        private readonly string _emoji;
        private readonly string _interfaceName;

        public ConstructorLoggingMixin(CallLogger logger, string emoji, string interfaceName)
        {
            _logger = logger;
            _emoji = emoji;
        ...
        public void ConstructorCalledWith(ConstructorParameterInfo[] parameters)
        {
            ...
            // Get the interface name from context if available
            var interfaceName = CallLogFormatterContext.GetCurrentInterfaceName();
            if (string.IsNullOrEmpty(interfaceName))
            {
            ...
            callLogger.log(_emoji, $"{_interfaceName} constructor called with");
        }
    }
}