...
## Overview
Comprehensive integration testing of the complete ObjectFactory → CallLogger → Parrot pipeline to ensure the object ID tracking system works seamlessly across all components.
...
var wrappedUserService = logger.Wrap<IUserService>(userService, "👤", factory);
var wrappedEmailService = logger.Wrap<IEmailService>(emailService, "📧", factory);
...
```csharp
// Pattern: Register → Execute → Log → Verify → Replay
[Fact]
...
```
SpecRec.Tests/
├── ObjectFactoryIdTests.cs           # Phase 1
├── CallLoggerIdFormattingTests.cs    # Phase 2  
├── ParrotObjectIdTests.cs            # Phase 3
├── ObjectIdWorkflowIntegrationTests.cs # Phase 4
└── fixtures/
    ├── integration-scenarios/
    │   ├── BasicServiceCollaboration.verified.txt
    │   ├── ComplexObjectPassing.verified.txt
    │   └── CollectionHandling.verified.txt
    └── test-services/
        ├── UserService.cs
        ├── EmailService.cs
        └── DataService.cs
